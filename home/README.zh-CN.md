...
用户级配置,由 home-manager 管理。按用户装的应用、dotfiles、主题、shell 配置 —— 原则上不需要 root 就能装的东西都在这里。
...
叠加关系:**core ⊂ tui ⊂ gui**。`home/linux/` 下**没有** `default.nix` ——主机必须显式选择。

## 延伸阅读

- [home/base/README.zh-CN.md](./base/README.zh-CN.md) —— 跨平台规则,`core/` vs `tui/` vs `gui/`
  怎么选
