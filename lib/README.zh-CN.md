...
主机文件只负责**拼 list**:`nixos-modules` + `home-modules`,然后把list 丢给
`mylib.nixosSystem`。nixpkgs / home-manager 的对接细节只在一个地方,主机文件就能只有几十行。

以后加新平台(darwin、riscv 等)或新部署模式(colmena、nixos-generators生成镜像)时,在这里**加新的工厂函数**,而不是在每个主机文件里重复 wiring。

## 新增工具函数

扔一个 `<name>.nix` 导出辅助函数,在 `default.nix` 里加一行引用, 全项目就能 `mylib.<name>` 用到了。
