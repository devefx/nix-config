...
`outputs/default.nix` imports `./vars` and passes the result into every module as `myvars` via
`specialArgs`. Any module can destructure:
...
Add a field to `default.nix`. Anything here is instantly visible to every NixOS / home-manager
module in the flake.

Before moving a string into `vars/`, ask: **does it appear in more than one module?** If not, keep
it local — `vars/` is for truly global facts (identity, keys, domain, timezone), not a catch-all
config bag.
